...
wgac <- coverage(wga)
bins <- tileGenome(seqlengths(gr), tilewidth=wgaTileWidth, cut.last.tile.in.chrom=TRUE)
...
# look for the regions that appear off-target
offR <- wgaba[which(wgaba$binned_cov > (offTargetThreshold*wga.cov))]
...
if (file.exists(targetFile) & !force) {
  ...
  backgroundRx <- GenomicRanges::reduce(backgroundR)
  offRx <- GenomicRanges::reduce(offR)
  
  backgroundMappings <- fixBamFileColumns(as.data.frame(t(as.data.frame(mclapply(seq_along(backgroundRx), extractMappingInfo, xr=backgroundRx, mc.cores=min(detectCores()-1,8)))), stringsAsFactors=FALSE, row.names=seq_along(backgroundRx)))
  
  offTargetMappings <- fixBamFileColumns(as.data.frame(t(as.data.frame(mclapply(seq_along(offRx), extractMappingInfo, xr=offRx, mc.cores=min(detectCores()-1,8)))), stringsAsFactors=FALSE, row.names=seq_along(offRx)))
  
  targetFlankingMappings <- fixBamFileColumns(as.data.frame(t(as.data.frame(mclapply(seq_along(fr), extractMappingInfo, xr=fr, mc.cores=min(detectCores()-1,8)))), stringsAsFactors=FALSE, row.names=seq_along(fr)))
  
  onTargetMappings <- fixBamFileColumns(as.data.frame(t(as.data.frame(mclapply(seq_along(br), extractMappingInfo, xr=br, mc.cores=min(detectCores()-1,8)))), stringsAsFactors=FALSE, row.names=seq_along(br)))
  
  save(backgroundMappings, offTargetMappings, targetFlankingMappings, onTargetMappings, file=targetFile)
} 
...
summary.df <- cbind(collateMappingCharacteristics(backgroundMappings, unmappedReads), 
                    collateMappingCharacteristics(offTargetMappings),
                    collateMappingCharacteristics(targetFlankingMappings),
                    collateMappingCharacteristics(onTargetMappings))
...
bygene <- cbind(onTargetMappings, geneName=unlist(bed[,4]))[,c("geneName", "chrId", "readStarts", "basesReadsStarted", "readLen", "readQ", "mapq", "meanCov")]
bygene$fwd <- round(as.numeric(onTargetMappings$plusStrand)/as.numeric(onTargetMappings$readStarts), digits=2)
...
aggregatedCov <- bind_rows(mclapply(seq_along(br), aggregateDepthInfo, xr=br, mc.cores=min(detectCores()-1,8)), .id = "column_label")

singlePlot <- function(geneName, covData) {
  covData <- covData[which(covData$gene==geneName),]
  ggplot(covData)+
    geom_line(aes(x=pos,y=binned_cov), size=0.5, colour=brewer.pal(6, "Paired")[2])+
    xlab(paste("Position on chromosome",unique(covData$chrId))) + 
