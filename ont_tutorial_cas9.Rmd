...
singlePlot <- function(geneName, covData) {
  ...
  ggplot(covData)+geom_line(aes(x=pos,y=value), size=0.5, colour=brewer.pal(6, "Paired")[2])+xlab(paste("Position on chromosome",unique(covData$chrId))) + ylab("Depth of Coverage (X)") + labs(title=paste("Plot showing depth of coverage vs position for target", geneName)) + scale_x_continuous(labels = comma)
}
...
plotCoverageMegablock <- function(covData) {
  covData$rpos <- (as.numeric(covData$rpos))/1000000
  suppressWarnings(megadepthplot <- ggplot(covData, aes(rpos, value)) + geom_line(colour=brewer.pal(6, "Paired")[2]) + facet_grid(rows = vars(row), cols=vars(col)) + theme(axis.text.x = element_text(angle = 90, hjust = 1), strip.text.y = element_blank(), strip.text.x = element_blank()) + xlab("Position across target region (Mb)") + ylab("Depth of Coverage (X)") + labs(title="Plot showing depth of coverage vs position for target regions") + geom_text(aes(x,y,label=lab), data=legendDF, vjust=1, hjust=1, size=3.5) + theme(plot.title = element_text(size=11)))
...
aggregateDepthInfo <- function(x, xr) {
    ...
    params=ScanBamParam(which=GRanges(seqnames = "1", ranges = IRanges(start = 1, end = 155104999)), 
                      what=c("qname", "strand", "pos", "cigar"), 
                      flag=scanBamFlag(isSupplementaryAlignment=FALSE, isSecondaryAlignment=FALSE))
  SeqCigar <- as.data.frame(scanBam(BamFile(mappedBam), param=params)[[1]])
  cov <- GAlignments(seqnames=SeqCigar$qname, pos=SeqCigar$pos, strand=SeqCigar$strand, cigar=as.character(SeqCigar$cigar))
  dd <- as.data.frame(coverage(cov, shift=0, width=(155104999), method="hash"))
  dd$pos <- seq_along(dd$value)+start
  dd$rpos <- seq_along(dd$value)
  dd$gene <- geneId
  dd$chrId <- chrId
