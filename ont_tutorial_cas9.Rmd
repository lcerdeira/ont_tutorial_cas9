...
ontarget.meta <- file.path(r_results, paste0(study,"_ontarget.xls"))
write.csv(as.data.frame(ontargetUniverse)[,c(1,2,3,4,6,8,14,16,23)], ontarget.meta)
```


* The list of off-target read Ids can be found in the file **`Analysis/OnTarget/<TARGETNAME>.mappedreads`**
...
The final plot is a tiled plot that can be used to present multiple depth-of-coverage plots in a single figure. This looks great for studies that are evaluating multiple targets; if you have a single target region this may appear a little empty. 
...
offtargtop$dmean <- round(offtargtop$dmean, digits=2)
offtargtop$meanreadlen <- scales::comma_format()(offtargtop$meanreadlen)
offtargtop$mapq <- round(offtargtop$mapq, digits=2)

colnames(offtargtop) <- c("chrId", "start", "end", "mean coverage", "reads in segment", "mean read length", "reads on FWD%", "MAPQ")
...
offtarget.meta <- file.path(r_results, paste0(study,"_offtarget.xls"))
write.csv(as.data.frame(offtargetUniverse)[,c(1,2,3,4,6,8,14,16,23)], offtarget.meta)
```


* The list of off-target read Ids can be found in the file **`Analysis/OnTarget/OffTarget.mappedreads`**
* The **`fastq`** sequence file containing the raw sequence reads corresponding to these Ids can be found in the file **`Analysis/OnTarget/OffTarget.fastq`**
* The coordinate information for the off-target regionms can be found in the file **`r offtarget.meta`**
